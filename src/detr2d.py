...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, labels: torch.Tensor):
        ...
        ), "batch_size of logits and labels must be same"
        ...
        indices = [
            linear_sum_assignment(c[i]) for i, c in enumerate(cost.split(sizes, -1))
        ]
        return [
            (
...
class DETRHead(nn.Module):
    def __init__(
        ...
        self.linear_bbox = nn.Linear(
            hidden_dim, 4
        )  # [onset, onset_var, wakeup, wakeup_var]

        self.query = nn.Parameter(
            torch.rand(1, max_det, hidden_dim)
        )  # [max_det, hidden_dim]
...
class DETR2DCNN(nn.Module):
    ...
    def forward(
        ...
        if do_mixup and labels is not None:
            x, _, labels, _ = augmentations.mixup(x, labels)
...
def logits_to_proba_per_step(logits: torch.Tensor, org_duration: int) -> torch.Tensor:
    ...
    gaussian_onset = gaussian(
        x, objectness, onset_mu, onset_var
...
def correct_labels(labels: torch.Tensor, org_duration: int) -> torch.Tensor:
    ...
    return torch.stack(
        [onset_label, wakeup_label], dim=-1
    )  # (batch_size, org_duration, 2)
